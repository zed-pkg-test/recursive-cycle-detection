...
- `zed-pkg/zed-interfaces`
    ...
    1. Verify direct and indirect cycle detection without partial links or corrupt CAS state across the supported happy-path states and canonical fixtures.
2. Verify direct and indirect cycle detection without partial links or corrupt CAS state under retries, interruption, concurrency, offline operation, or partial failure.
3. Verify direct and indirect cycle detection without partial links or corrupt CAS state preserves authorization, idempotency, integrity, observability, and actionable failure classification.
